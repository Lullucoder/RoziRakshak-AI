...
import { FraudSignalVector } from '../types/fraud';
import { queryCollection, Timestamp } from '../utils/firestore';
import { logger } from '../utils/logger';

/**
 * Build the complete fraud signal vector with safe defaults
 */
export async function buildSignalVector(context: ClaimContext): Promise<FraudSignalVector> {
  ...
  const recentClaims = await queryCollection<any>(
    'claims',
    [
      { field: 'workerId', operator: '==', value: worker.uid },
      { field: 'createdAt', operator: '>=', value: Timestamp.fromDate(sevenDaysAgo) }
    ]
  );
  ...
  const days_since_registration = Math.floor(
    (Date.now() - worker.joinedDate.toMillis()) / (24 * 60 * 60 * 1000)
  );
  ...
  const signalVector: FraudSignalVector = {
